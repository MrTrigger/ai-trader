...
/// synchronous.
fn with_registry<T>(
    f: impl FnOnce(&records::Registry) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<T, records::RecordsError>> + '_>>,
) -> Result<T, String> {
    let url = std::env::var("DATABASE_URL")
        .map_err(|_| "DATABASE_URL is not set — the fleet view needs the identity registry".to_string())?;
    let rt = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .map_err(|e| e.to_string())?;
    rt.block_on(async {
        let reg = records::Registry::connect(&url).await.map_err(|e| e.to_string())?;
        f(&reg).await.map_err(|e| e.to_string())
    })
}

fn heartbeat_age_seconds(iso: &str) -> Option<i64> {
    let ts = time::OffsetDateTime::parse(iso, &time::format_description::well_known::Rfc3339).ok()?;
...
/// GET /api/bots
pub fn list(repo_root: &Path) -> Result<Value, String> {
...
pub fn set_halt(repo_root: &Path, bot_id: &str, halt: bool, reason: &str, by: &str) -> Result<Value, String> {
    ...
    let mut current: Value = std::fs::read_to_string(&path)
        ...
        .unwrap_or_else(|| json!({}));
    let obj = current.as_object_mut().ok_or("control.json is not an object")?;
